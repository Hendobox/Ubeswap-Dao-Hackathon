...
describe("DAO Milestones ", function () {
  ...
  async function deployFixture() {
    ...
    const contract_ = await DAOMilestones.deploy("UBE NFT", "UBENFT", owner_.address);
  ...
  describe("Deployment", function () {
    ...
    let contract:any, amount:any, owner:any, otherAccount:any, addr0:any, milestones:array;
    
    before(async function() {
      const {contract_, amount_, owner_, otherAccount_, addr0_}  = await loadFixture(deployFixture);
    ...
    it("Should set project correctly", async function () {
      ...
      const val: array = await contract.getProject(1);
    ...
    it("Should properly deposit and start project", async function () {
      ...
      await expect(contract.revokeProject(1, addr0)).to.revertedWithCustomError(contract, "PROJECT_NOT_SET");

      const val: array = await contract.getProject(1);

      await expect(
        contract.deposit(1, { value: 1000000})
        ).to.rejectedWith("INVALID_AMOUNT_PASSED"
      );
      ...
      await expect(contract.deposit(1, {value: val[3]})).to.revertedWithCustomError(contract, "PROJECT_ALREADY_SET");
    })

    it("Should properly resolve milestones", async function () {
      await expect(contract.resolveMilestone(1, 0, true, owner.address)).to.revertedWithCustomError(contract,"MILESTONE_NOT_REACHED")
      
      await expect(contract.resolveMilestone(1, 0, false, addr0)).to.revertedWith("NULL_ADDRESS")

      await contract.resolveMilestone(1, 0, false, owner.address)

      const bal: number = await ethers.provider.getBalance(contract.address);

      expect(bal.toString()).to.equal((ethers.BigNumber.from(amount).sub((ethers.BigNumber.from(amount).div(4)))).toString())

      await expect(contract.resolveMilestone(1, 0, true, owner.address)).to.revertedWithCustomError(contract,"MILESTONE_ALREADY_CLOSE")

      // increment time
      time.increase(milestones[1].timestamp);

      const bal1: number = await ethers.provider.getBalance(otherAccount.address);

      await contract.resolveMilestone(1, 1, true, owner.address)

      const bal2: number = await ethers.provider.getBalance(otherAccount.address);

      expect(ethers.BigNumber.from(bal1).add(ethers.BigNumber.from(amount).div(4))).to.equal(bal2)
    })

    it("Should properly revoke projects", async function() {
      await expect(contract.revokeProject(1, addr0)).to.revertedWith("NULL_ADDRESS");

      const bal1: number = await ethers.provider.getBalance(owner.address);

      await contract.revokeProject(1, owner.address)

      const bal2: number = await ethers.provider.getBalance(owner.address);

      expect(Number(bal2)).to.be.greaterThan(Number(bal1))
      
      await expect(contract.revokeProject(1, addr0)).to.revertedWithCustomError(contract,"MILESTONE_ALREADY_REVOKED");

      const val: array = await contract.getProject(1);

      expect(val[4].toString()).to.equal(ethers.BigNumber.from(amount).div(4))
      expect(val[5].toString()).to.equal(ethers.BigNumber.from(amount).sub(ethers.BigNumber.from(amount).div(4)))
